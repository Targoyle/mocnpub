...
**残りの実装機能**：
- **テストコード**：品質保証（Phase 2 実装時の安心感）← **次はこれ！**
- **継続モード**：一晩動かして複数の候補を集める
- **複数 prefix 指定**：欲しい prefix の組み合わせを探す
- **ベンチマーク**：パフォーマンス測定
...
**技術選択**：
- **rayon** クレート（Rust のデファクトスタンダード並列化ライブラリ）
  - データ並列処理が簡潔に書ける
  - ワークスティーリングで効率的
- または **std::thread** + **crossbeam-channel**（より細かい制御が必要な場合）

**期待される高速化**：
- CPU コア数に応じて線形に高速化（理想的には N倍）
- 例：8コア CPU → 約 8倍高速化（5〜7万 keys/sec → 40〜56万 keys/sec）
...
**Phase 2（中優先度）**：
3. **テストコード** 🧪 **← 次はこれ！**
   - 品質保証（Phase 2 実装時の安心感）
   - `validate_prefix()` のテスト（無効文字、大文字、空文字）
   - `pubkey_to_npub()` のテスト（既知の公開鍵 → 既知の npub）
   - `seckey_to_nsec()` のテスト
   - 複数 prefix マッチングのテスト（Phase 2 実装後）
4. **継続モード** 💪
   - 実用性が高い
   - 一晩動かす用途
5. **複数 prefix の OR 指定** 🎯
