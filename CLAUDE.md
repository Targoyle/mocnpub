...
**現在のパフォーマンス**：
- CPU（16スレッド）: ~70,000 keys/sec
- GPU: ~1,160,000 keys/sec（**16 倍** 🔥）

**さらなる高速化を目指す**：現状の **100 倍以上** を目標！
...
**Phase 1（最優先）**：
1. **連続秘密鍵 + PointAdd** — 最もインパクト大
2. **Mixed Addition** — 連続秘密鍵と組み合わせて効果増

**Phase 2（高優先）**：
3. **Montgomery's Trick** — 逆元計算のバッチ処理
4. **bech32 前の prefix マッチング** — 転送量削減
