...
| **GPU + Z² 累積積戦略（Montgomery's Trick で ModSquare 削減）** | **5.80B keys/sec** | **82,857x** 🔥 |
...
| **Z² 累積積戦略** | **+0.5%**（Z の代わりに Z² を累積、Phase 3 の ModSquare 1600回を削減） 🔥 | ✅ 完了 |
...
| **SoA 最適化（グローバルメモリ版）** | キャッシュミス多発、VRAM 消費大、batch_size を上げられない（3.09B → 2.70B）。**ただしローカル配列の SoA 化は効果あり**（Step 29: +1.1%、アドレス計算の簡素化による命令数・レジスタ削減）|
...
| **`#pragma unroll` prefix matching** | **効果なし（2025-12-26）**: `#pragma unroll 4` を prefix matching ループに追加したが、誤差範囲内の変化。constant memory がボトルネックの場合、ループアンローリングの効果は限定的 |
