...
### ncu プロファイリング結果（2025-11-29, 2025-11-30）
...
| Occupancy | 33% | レジスタ制限 |
| Registers/Thread | 130 | 多いが最適 |
| Local Memory Spilling | 0% | スピルなし ✅ |

**メモリコアレッシング問題を検出**（2025-11-30）：
- local loads: 32バイト中1バイトしか活用できていない（Est. Speedup ~10%）
- local stores: 32バイト中1バイトしか活用できていない（Est. Speedup ~12%）
- **原因**: Montgomery's Trick のローカル配列（`X_arr`, `Y_arr`, `Z_arr`, `c`）が AoS レイアウト

**結論**: Compute bound。SoA 最適化で ~20% 改善の余地あり。
...
**メモリコアレッシング最適化の実装計画**：
- ローカル配列（`X_arr`, `Y_arr`, `Z_arr`, `c`）をグローバルメモリに移動
- SoA（Structure of Arrays）レイアウトでメモリコアレッシング最適化
