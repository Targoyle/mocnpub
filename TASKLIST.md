...
| **`_Reduce512` Step 3 ブランチレス化** | **逆効果（2025-12-22）**: テーブル + マスク選択で for ループを置換したが -9%（5.395B → 4.899B）。`temp[4] ≈ 0` かつ `temp[0..3] < p` が多く分岐予測が効いていた。`_ModAdd`/`_ModSub` と違い入力に強い偏りがある |
| **`_Sub256` borrow 正規化削除** | **逆効果（2025-12-22）**: `& 1` を削除して 0/0xFFFFFFFF をそのまま返すようにしたが -1.6%（5.499B → 5.411B）。`& 1` で 0/1 に正規化することで、コンパイラが `-borrow` を効率的に最適化できる（boolean として認識）。`!!` や符号拡張より `& 1` + `-` の方が速い |
| **`#pragma unroll` prefix matching** | **効果なし（2025-12-26）**: `#pragma unroll 4` を追加したが誤差範囲内。constant memory がボトルネックの場合、ループアンローリングの効果は限定的 |
