...
// ============================================================================
// cuRAND-based Key Generation with Prefix Matching
// ============================================================================

/**
 * Add a small offset (u32) to a 256-bit number
 * Used to compute actual secret key = base_key + offset
 */
__device__ void _AddOffset256(const uint64_t base[4], uint32_t offset, uint64_t result[4])
{
    uint64_t carry = offset;
    for (int i = 0; i < 4; i++) {
        uint64_t sum = base[i] + carry;
        carry = (sum < base[i]) ? 1 : 0;
        result[i] = sum;
    }
}

/**
 * GPU kernel for key generation and prefix matching using cuRAND
 *
 * This kernel generates secret keys directly on the GPU using cuRAND,
 * eliminating CPU->GPU transfer overhead.
 *
 * Input:
 *   - seed: Random seed for cuRAND (should be different per batch)
 *   - patterns: prefix bit patterns [num_prefixes] (upper bits of x-coordinate)
 *   - masks: prefix bit masks [num_prefixes] (upper bits set to 1)
 *   - num_prefixes: number of prefixes to match
 *   - num_threads: number of threads
 *   - keys_per_thread: consecutive keys per thread (max 256)
 ...
 *
 * Output:
 *   - matched_seckeys: secret keys of matched keys [max_matches * 4]
 *   - matched_pubkeys_x: x-coordinates of matched pubkeys [max_matches * 4]
 *   - match_count: number of matches found (atomic counter)
 */
extern "C" __global__ void generate_and_match_curand(
    uint64_t seed,
    const uint64_t* patterns,
    const uint64_t* masks,
    uint32_t num_prefixes,
    uint64_t* matched_seckeys,
    uint64_t* matched_pubkeys_x,
    uint32_t* match_count,
    uint32_t num_threads,
    uint32_t keys_per_thread,
...
)
{
    ...
    // Generate 256-bit secret key using cuRAND
    uint64_t k[4];
    for (int i = 0; i < 4; i++) {
        uint32_t lo = curand(&state);
        uint32_t hi = curand(&state);
        k[i] = ((uint64_t)hi << 32) | lo;
    }
    ...
    for (int32_t i = n - 1; i >= 0; i--) {
        ...
        for (uint32_t p = 0; p < num_prefixes; p++) {
            if ((x_upper & masks[p]) == (patterns[p] & masks[p])) {
                ...
                uint32_t slot = atomicAdd(match_count, 1);
                if (slot < max_matches) {
                    // Compute actual secret key: k + offset
                    uint64_t actual_seckey[4];
                    _AddOffset256(k, i, actual_seckey);

                    // Store matched secret key
                    for (int j = 0; j < 4; j++) {
                        matched_seckeys[slot * 4 + j] = actual_seckey[j];
                    }
                    // Store matched public key x-coordinate
                    for (int j = 0; j < 4; j++) {
                        matched_pubkeys_x[slot * 4 + j] = x[j];
                    }
                }
