...
/// Match result from cuRAND-based GPU key generation and prefix matching
#[derive(Debug, Clone)]
pub struct CurandMatch {
    /// Secret key (256-bit, little-endian limbs)
    pub seckey: [u64; 4],
    /// Public key x-coordinate (256-bit, little-endian limbs)
    pub pubkey_x: [u64; 4],
}
...
// ============================================================================
// cuRAND-based Key Generation and Prefix Matching
// ============================================================================

/// Generate keys and match prefixes using cuRAND on GPU
///
/// This function generates secret keys directly on the GPU using cuRAND,
/// eliminating CPU->GPU transfer overhead. It returns matched keys with
/// both secret key and public key.
///
/// # Arguments
/// * `ctx` - GPU context
/// * `seed` - Random seed for cuRAND (should be different per batch)
/// * `num_threads` - Number of GPU threads (each generates keys_per_thread keys)
/// * `keys_per_thread` - Consecutive keys per thread (max 256)
/// * `prefix_bits` - Prefix bit patterns from `prefixes_to_bits()`
/// * `max_matches` - Maximum number of matches to store
///
/// # Returns
/// Vector of `CurandMatch` containing matched secret keys and public keys
pub fn generate_and_match_curand(
    ctx: &Arc<CudaContext>,
    seed: u64,
    num_threads: usize,
    keys_per_thread: u32,
    prefix_bits: &[(u64, u64, u32)],
    max_matches: u32,
) -> Result<Vec<CurandMatch>, Box<dyn std::error::Error>> {
    let num_prefixes = prefix_bits.len();
    if num_prefixes == 0 {
        return Err("No prefixes provided".into());
    }
    ...
    let ptx_code = include_str!("../cuda/secp256k1.ptx");
    let module = ctx.load_module(Ptx::from_src(ptx_code))?;
    let kernel = module.load_function("generate_and_match_curand")?;
    ...
    // Allocate device memory for outputs
    let mut matched_seckeys_dev = stream.alloc_zeros::<u64>(max_matches as usize * 4)?;
    let mut matched_pubkeys_x_dev = stream.alloc_zeros::<u64>(max_matches as usize * 4)?;
    let mut match_count_dev = stream.alloc_zeros::<u32>(1)?;
    ...
    builder.arg(&mut masks_dev);
    builder.arg(&num_prefixes_u32);
    builder.arg(&mut matched_seckeys_dev);
    builder.arg(&mut matched_pubkeys_x_dev);
    builder.arg(&mut match_count_dev);
    builder.arg(&num_threads_u32);
    builder.arg(&keys_per_thread);
    ...
    // Copy results back
    let matched_seckeys_flat = stream.memcpy_dtov(&matched_seckeys_dev)?;
    let matched_pubkeys_x_flat = stream.memcpy_dtov(&matched_pubkeys_x_dev)?;

    // Build result vector
    let mut results = Vec::with_capacity(match_count);
    for i in 0..match_count {
        let mut seckey = [0u64; 4];
        seckey.copy_from_slice(&matched_seckeys_flat[i * 4..(i + 1) * 4]);

        let mut pubkey_x = [0u64; 4];
        pubkey_x.copy_from_slice(&matched_pubkeys_x_flat[i * 4..(i + 1) * 4]);

        results.push(CurandMatch {
            seckey,
            pubkey_x,
        });
    }
